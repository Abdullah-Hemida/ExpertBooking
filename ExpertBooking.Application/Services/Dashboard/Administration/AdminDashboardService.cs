...
namespace ExpertBooking.Application.Services.Dashboard.Administration
{
    public class AdminDashboardService : IAdminDashboardService
    {
        ...
        public async Task<ServiceResponse<List<CategoryBookingStatDto>>> GetBookingStatsByCategoryAsync()
        {
            var stats = await _bookingRepository.GetBookingStatsByCategoryAsync();
            return ServiceResponse<List<CategoryBookingStatDto>>.Success(stats);
        }

        public async Task<ServiceResponse<List<TopRatedExpertDto>>> GetTopRatedExpertsAsync(int topN = 5)
        {
            var experts = await _reviewRepository.GetTopRatedExpertsAsync(topN);
            return ServiceResponse<List<TopRatedExpertDto>>.Success(experts);
        }
    }
}
